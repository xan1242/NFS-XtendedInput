...
void(__thiscall* CustomTuningScreen_NotificationMessage)(void* thethis, unsigned int unk1, void* FEObject, unsigned int unk2, unsigned int unk3) = (void(
    __thiscall*)(void*, unsigned int, void*, unsigned int, unsigned int))0x005A9910;
...
void(__thiscall* SelectCarCameraMover_SetHRotateSpeed)(void* SelectCarCameraMover, float speed,
                                                       bool activate)                 = (void(__thiscall*)(void*, float, bool))SELECTCAR_SETHROTATESPEED_ADDR;
void(__thiscall* SelectCarCameraMover_SetVRotateSpeed)(void* SelectCarCameraMover, float speed,
                                                       bool activate)                 = (void(__thiscall*)(void*, float, bool))SELECTCAR_SETVROTATESPEED_ADDR;
void(__thiscall* SelectCarCameraMover_SetZoomSpeed)(void* SelectCarCameraMover, float speed,
                                                    bool activate)                    = (void(__thiscall*)(void*, float, bool))SELECTCAR_SETZOOMSPEED_ADDR;
void(__thiscall* FeGarageMain_ZoomCameraView)(void* FEGarageMain, float speed,
                                              bool activate)                          = (void(__thiscall*)(void*, float, bool))FEGARAGEMAIN_ZOOMCAMERAVIEW_ADDR;
...
void FEngSetVisible(void* FEObject) { return FE_Object_SetVisibility(FEObject, true); }
void FEngSetInvisible(void* FEObject) { return FE_Object_SetVisibility(FEObject, false); }

#endif

char* cFEng_FindPackageWithControl_Name() {
  int pkg = cFEng_FindPackageWithControl();
  if (pkg) return *(char**)(pkg + 0xC);
  return NULL;
}

// fast version using hashes instead of string comparison
bool cFEng_IsPackageInControl_Fast(unsigned int pkg_name_hash) {
  if (bStringHash(cFEng_FindPackageWithControl_Name()) == pkg_name_hash) return true;
  return false;
}
...
unsigned int __stdcall FEngGetTextureHash(unsigned int FEImage) {
  ...
  if (FEImage) result = *(int*)(FEImage + 36);
...
int __stdcall FEngFindImage(char* pkgname, int hash) {
  ...
  if (!result || *(int*)(result + 24) != 1) result = 0;
  return result;
}
...
int __stdcall cFEng_FindPackage(char* pkg) { return _cFEng_FindPackage(*(void**)CFENG_INSTANCE_ADDR, pkg); }

bool __stdcall cFEng_IsPackageInControl(char* pkg) { return _cFEng_IsPackageInControl(*(void**)CFENG_INSTANCE_ADDR, pkg); }

void FEngSetVisible_Name(char* pkg_name, unsigned int obj_hash) {
  FEObject* obj = FEngFindObject(pkg_name, obj_hash);
  if (obj) FEngSetVisible(obj);
}

void FEngSetInvisible_Name(char* pkg_name, unsigned int obj_hash) {
  void* obj = FEngFindObject(pkg_name, obj_hash);
  if (obj) FEngSetInvisible(obj);
}
  ...
  if ((*(int*)GAMEFLOWMANAGER_STATUS_ADDR != 0) && bGlobalDoPolling)  // don't execute until we're in the game... fixes BSOD crash on old ATI video drivers...
  {
    ...
    *(int*)FEMOUSECURSOR_CARORBIT_Y_ADDR = MousePos.y - *(int*)FEMOUSECURSOR_Y_ADDR;
    ...
    if ((MousePos.x != *(int*)FEMOUSECURSOR_X_ADDR) || (MousePos.y != *(int*)FEMOUSECURSOR_Y_ADDR))
      ...
      if ((TimeSinceLastMouseMovement + MOUSEHIDE_TIME) < timeGetTime()) bShowMouse = false;
    }
#ifdef GAME_MW
    if (*(int*)GAMEFLOWMANAGER_STATUS_ADDR == 6) {
      if (*(bool*)FEMOUSECURSOR_ISHIDDEN_ADDR && cFEng_IsPackageInControl_Fast(WORLDMAPMAIN_FNG_NAMEHASH)) bShowMouse = false;
    }
#else
#ifdef GAME_CARBON
    if (*(bool*)FEMOUSECURSOR_ISHIDDEN_ADDR && cFEng_IsPackageInControl_Fast(WORLDMAPMAIN_FNG_NAMEHASH)) bShowMouse = false;
    ...
    if (MouseWheelValue) *(int*)FEMOUSEWHEEL_ADDR = MouseWheelValue;
    MouseWheelValue = 0;
  }
}
...
bool bInPCControllerConfig() {
  return cFEng_IsPackageInControl_Fast(NFS_HASH("FeControllerConfig_PC.fng")) ||
         cFEng_IsPackageInControl_Fast(NFS_HASH("FeControllerConfig_PC_ReMap_Dialog.fng"));
}
#endif
...
void __stdcall cFEng_Service_Hook(int unk1, int unk2) {
  ...
  if (FEPackage) {
    while (FEPackage != NextFEPackage) {
      ...
      if (NextFEPackage == FEPackage) break;
